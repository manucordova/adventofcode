...
class AOCConnector:
    ...
    def __init__(self, year: int, day: int):
        ...
        if year > self.max_year:
            raise InvalidDateError(
                f"The requested year is in the future! We are still in {self.max_year}"
            )

        if day < self.min_day or day > self.max_day:
            raise InvalidDateError(f"Invalid day: {day}")

        if year == self.max_year and (
            self.cur_date.month < 12 or day > self.cur_date.day
        ):
    ...
    def _get_instruction_page(self):
        ...
        response = req.get(url, cookies={"session": self.SESSION_TOKEN})
    ...
    def _make_input(self, root_dir):
        ...
        input_response = req.get(url, cookies={"session": self.SESSION_TOKEN})
    ...
    def _make_python_canevas(self, canevas: str, root_dir):
    ...
    def initialize(
        self,
        canevas: str,
        root: str = pl.Path(".").resolve(),
        dir_name: str = "challenges",
        force: bool = False,
    ):
        ...
        if os.path.exists(day_path) and not force:
            print(
                f"Directory {day_path.resolve()} already exists! Use the -f flag to overwrite it."
            )
            return
    ...
    def get_level(self):
        ...
        if (
            ...
            in self.instruction_parser.get_content()
        ):
            level = 3
...
class HTMLParser:
    ...
    def _has_tag(self, content: str, tag: str):
        return f"<{tag}" in content and f"</{tag}>" in content

    def _get_inner_tag(self, content: str, tag: str):

        if f"<{tag}>" in content and f"</{tag}>" in content:
            return content.split(f"<{tag}>")[1].split(f"</{tag}>")[0], f"</{tag}>".join(
                content.split(f"</{tag}>")[1:]
            )

        elif f"<{tag}" in content and f"</{tag}>" in content:

            inner_content = ">".join(content.split(f"<{tag}")[1].split(">")[1:])
            return inner_content.split(f"</{tag}>")[0], f"</{tag}>".join(
                content.split(f"</{tag}>")[1:]
            )
